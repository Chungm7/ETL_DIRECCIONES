...
-- Tablas de direcciones origen:
--   - 'direcciones_actual'   : Tabla de trabajo y enriquecimiento in-place estándar.
--   - 'direcciones_licencias': Tabla de inserción original con las 42,462 licencias.
--
-- NOTA DE CARGA MASIVA:
-- Los 42,462 registros se cargan automáticamente al iniciar el contenedor
-- a través del script complementario:
--   -> docker/initdb/02_direcciones_licencias.sql
-- Adicionalmente, si existiera '/data_import/Direcciones_.csv', se importa vía COPY.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS public.direcciones_licencias (
    id_licencia     INTEGER NOT NULL,
    emp_direccion   VARCHAR(255),
    CONSTRAINT pk_direcciones_licencias PRIMARY KEY (id_licencia)
);

-- Carga opcional desde CSV si está disponible en el volumen
DO $$
BEGIN
    IF pg_stat_file('/data_import/Direcciones_.csv', true) IS NOT NULL THEN
        ...
        INSERT INTO public.direcciones_licencias (id_licencia, emp_direccion)
        SELECT id_licencia, emp_direccion FROM public.direcciones_actual
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Aviso durante verificación de /data_import/Direcciones_.csv: %', SQLERRM;
END $$;
