...
def cmd_run_etl(
    ...
    if RICH_AVAILABLE and console:
        ...
        table.add_row("✅ Validados en Catastro Oficial", f"{summary.valid_processed_records} ({(summary.valid_processed_records / total_proc) * 100:.1f}%)")
        table.add_row("⚠️ Observados / No Procesados", f"{summary.observed_records} ({(summary.observed_records / total_proc) * 100:.1f}%)")
        ...
        print(f"Validados en Catastro   : {summary.valid_processed_records} ({(summary.valid_processed_records / total_proc) * 100:.1f}%)")
        print(f"Observados / No Proc.   : {summary.observed_records} ({(summary.observed_records / total_proc) * 100:.1f}%)")
